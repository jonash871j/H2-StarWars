...
namespace Starwars
{
    class Program
    {
        static List<Planet> planetsInput;
        static List<Planet> planetsOutput;
            ...
            while(true)
            {
                ...
                while (true)
                {
                    ...
                    switch (taskNumber)
                    {
                        ...
                        default: Console.WriteLine("Invalid task number"); break;
                    }

                    foreach (Planet planet in planetsOutput)
                    {
        ...
        static void Task1()
        {
            planetsOutput = planetsInput.FindAll(p => p.Name.StartsWith("M"));
        }
        static void Task2()
        {
            planetsOutput = planetsInput.FindAll(p => p.Name.ToLower().Contains("y"));
        }
        static void Task3()
        {
            planetsOutput = planetsInput.FindAll(p => p.Name.Length > 9 && p.Name.Length < 15);
        }
        static void Task4()
        {
            planetsOutput = planetsInput.FindAll(p => p.Name[1] == 'a' && p.Name.EndsWith("e"));
        }
        static void Task5()
        {
            planetsOutput = 
                planetsInput.FindAll(p => p.RotationPeriod > 40)
                .OrderBy(p => p.RotationPeriod).ToList();
        }

        static void Task6()
        {
            planetsOutput = 
                planetsInput.FindAll(p => p.RotationPeriod > 10 && p.RotationPeriod < 20)
                .OrderBy(p => p.Name)
                .ToList();
        }
        static void Task7()
        {
            planetsOutput = 
                planetsInput.FindAll(p => p.RotationPeriod > 30)
                .OrderBy(p => p.Name)
        ...
        {
            planetsOutput =
                planetsInput.FindAll(p => (p.RotationPeriod < 30 || p.SurfaceWater > 50) && p.Name.Contains("ba"))
                .OrderBy(p => p.Name)
        ...
        {
            planetsOutput =
                planetsInput.FindAll(p => p.SurfaceWater > 0)
        ...
        {
            planetsOutput =
                planetsInput.FindAll(p => p.Diameter != 0 && p.Population != 0)
                .OrderBy(p => Math.Pow(p.Diameter, 2) / p.Population)
                .ToList();
        }
        static void Task11()
        {
            List<Planet> planetsExcept = planetsInput.FindAll(p => p.RotationPeriod > 0);
            planetsOutput = 
                planetsInput.Except(planetsExcept)
                .ToList();
        }
        static void Task12()
        {
            List<Planet> planetList1 = planetsInput.FindAll(p => p.Name.ToLower().StartsWith("a") || p.Name.EndsWith("s"));
            List<Planet> planetList2 = planetsInput.FindAll(p => (p.Terrain ?? new List<string>()).Any(n => n.Contains("rainforest")));

            planetsOutput = planetList1.Union(planetList2).ToList();

        }
        static void Task13()
        {
            planetsOutput = 
                planetsInput.FindAll(p => (p.Terrain ?? new List<string>()).Any(n => n.Contains("desert")));
        }
        static void Task14()
        {
            planetsOutput =
                planetsInput.FindAll(p => (p.Terrain ?? new List<string>()).Any(n => n.Contains("swamp")))
        ...
        static void Task15()
        {
            ...
            planetsOutput =
                planetsInput.FindAll(p => doubleVowels.Any(p.Name.Contains) && !tripleVowels.Any(p.Name.Contains));
        }
        static void Task16()
        {
            string[] someDoubleCosonant =
            {
                "kk", "ll", "rr", "nn"
            };

            planetsOutput =
                planetsInput.FindAll(p => someDoubleCosonant.Any(p.Name.Contains))
